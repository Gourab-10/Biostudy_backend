...
router.post('/verify-code', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const notesAvailable = chapters
      .map((ch) => {
        const pdfs: string[] = JSON.parse(ch.pdfUrls || '[]');
        return {
          chapterId: ch.id,
...
router.get('/download/:chapterId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Validate the code
    const accessCode = await prisma.accessCode.findUnique({ where: { code: code as string } });
    ...
    const pdfs: string[] = JSON.parse(chapter.pdfUrls || '[]');
    const idx = parseInt(pdfIndex as string) || 0;
    ...
    if (pdfUrl.startsWith('/uploads/')) {
      ...
      res.setHeader(
        'Content-Disposition',
        `inline; filename="Ch${chapter.number}-${chapter.name.replace(/\s+/g, '-')}.pdf"`
      );
...
router.get('/view/:chapterId', async (req: Request, res: Response): Promise<void> => {
  const { code, pdfIndex } = req.query;
  const chapterId = req.params.chapterId as string;
  ...
  const downloadUrl = `/api/notes/download/${chapterId}?code=${encodeURIComponent(code as string)}&pdfIndex=${pdfIndex || 0}`;
...
router.post('/generate-code', authenticate, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Check uniqueness
    const existing = await prisma.accessCode.findUnique({ where: { code: generatedCode } });
    if (existing) {
...
router.get('/codes', authenticate, requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
